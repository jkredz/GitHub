...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        global thermalstate, canvas2,skintemp, interr, prototypestate, pstate, h, b, c, bc1,bc2,bc3,bc4,bc5,comflevel

        canvas2 = tk.Canvas(master=self, width=1000, height=900)
        ...
        bclabel = canvas2.create_text(50,300, fill="black", font=LARGE_FONT, anchor=tk.W, text="Comfort Level:")
        #bc = canvas2.create_text(370,300, fill="black", font=LARGE_FONT, anchor=tk.W, text=bccount)

        bc1=canvas2.create_rectangle(350,260, 470,330, fill='white')
        bc2=canvas2.create_rectangle(470,260, 590,330, fill='white')
        bc3=canvas2.create_rectangle(590,260, 710,330, fill='white')
        bc4=canvas2.create_rectangle(710,260, 830,330, fill='white')
        bc5=canvas2.create_rectangle(830,260, 950,330, fill='white')
        comflevel=canvas2.create_text(600,295, anchor=tk.W, font=('Helvetica', 24) , text=tstate)
...
def animate(i):
    ...
    print pstate
    if (pstate != 'Temp Recording'):
        comfortlevel(bccount)
    else:
        print tstate
        canvas2.itemconfig(comflevel, text=tstate, fill=b)
...
def comfortlevel(bccount):
    global tstate
    if bccount == -2:
        canvas2.itemconfig(bc1, fill="Blue")
        canvas2.itemconfig(bc2, fill="Cyan")
        canvas2.itemconfig(bc3, fill="White")
        canvas2.itemconfig(bc4, fill="White")
        canvas2.itemconfig(bc5, fill="White")
        tstate='Cold'
        canvas2.itemconfig(comflevel, text=tstate, fill='Blue')
    elif bccount == -1:
        canvas2.itemconfig(bc1, fill="white")
        canvas2.itemconfig(bc2, fill="Cyan")
        canvas2.itemconfig(bc3, fill="white")
        canvas2.itemconfig(bc4, fill="White")
        canvas2.itemconfig(bc5, fill="White")
        tstate='Cool'
        canvas2.itemconfig(comflevel, text=tstate, fill='Cyan')
    elif bccount == 0:
        canvas2.itemconfig(bc1, fill="white")
        canvas2.itemconfig(bc2, fill="white")
        canvas2.itemconfig(bc3, fill="white")
        canvas2.itemconfig(bc4, fill="white")
        canvas2.itemconfig(bc5, fill="White")
        tstate='Neutral'
        canvas2.itemconfig(comflevel, text=tstate, fill='Black')
    elif bccount == 1:
        canvas2.itemconfig(bc1, fill="white")
        canvas2.itemconfig(bc2, fill="white")
        canvas2.itemconfig(bc3, fill="white")
        canvas2.itemconfig(bc4, fill="Orange")
        canvas2.itemconfig(bc5, fill="White")
        tstate='Warm'
        canvas2.itemconfig(comflevel, text=tstate, fill='Orange')
    elif bccount == 2:
        canvas2.itemconfig(bc1, fill="white")
        canvas2.itemconfig(bc2, fill="white")
        canvas2.itemconfig(bc3, fill="white")
        canvas2.itemconfig(bc4, fill="Orange")
        canvas2.itemconfig(bc5, fill="Red")
        tstate='Hot'
        canvas2.itemconfig(comflevel, text=tstate, fill='Red')
    else:
        canvas2.itemconfig(bc1, fill="white")
        canvas2.itemconfig(bc2, fill="white")
        canvas2.itemconfig(bc3, fill="white")
