#!/usr/bin/env python
#gcode2lulzgcode_converter.py

import sys
line=0

#code to find nth needle in haystack
def find_nth(haystack, needle, n):
    start = haystack.find(needle)
    while start >= 0 and n > 1:
        start = haystack.find(needle, start+len(needle))
        n -= 1
    return start

print "Please enter the full name of the input raw .nc with extension at the prompt below"
#Raw_input is used to collect data from the user
inputfile = raw_input("> ")
outputfile = "lulz_print_"+inputfile.replace(".nc",".gcode")
extrudelen=0
z='Z' #look for string Z-axis
#heating temperatures  (Extruder,Bed)
#ABS = 240C 110C
#PLA 205C 60C
#HIP = 240C extruder, 110 BED

#Opens the filename entered above
with open(outputfile, 'w') as open_outputfile:
    open_outputfile.write("""M190 S110.000000   ;Wait for Bed Temperature to reach
M109 S240.000000                               ;Set Extruder Temperature and wait
;Sliced at: Sun 16-08-2015 13:30:33 for use with the LulzBot TAZ 5
;Basic settings: Layer height: 0.28 Walls: 1.05 Fill: 20
G21                     ;metric values
G90                     ;absolute positioning
M82                     ;set extruder to absolute mode
M107                    ;start with the fan off
G28 X0 Y0               ;move X/Y to min endstops
G28 Z0                  ;move Z to min endstops
G1 Z15.0 F10500         ;move the platform down 15mm
G92 E0                  ; zero the extruded length
G1 F200 E0              ; extrude 3mm of feed stock
G92 E0                  ; zero the extruded length again
G1 F10500               ; set travel speed
M203 X192 Y208 Z3       ; speed limits
M117 Printing...        ; send message to LCD
""")
    with open(inputfile, 'r') as file:
        for i in file:
            line=line+1
            if line < 19:
                pass
            elif line < 23:
                open_outputfile.write(i)  #Write the gcode from line 18 and onwards
            else:
                print i
                zindex=find_nth(i," ",4)+1
                print zindex
                i=i[:zindex]+"E"+str(extrudelen)+" "+i[zindex:] #%(extrudelen)
                #i=i.replace("\n","")+" E%s \n" %(extrudelen)
                extrudelen+=.1
                open_outputfile.write(i)  #Write the gcode from line 18 and onwards
    open_outputfile.write("""
M400
M104 S0                        ; hotend off
M140 S0                        ; heated bed heater off (if you have it)
M107                           ; fans off
G91                            ; relative positioning
G1 E-1 F300                    ; retract the filament a bit before lifting the nozzle, to release some of the pressure
G1 Z+0.5 E-5 X-20 Y-20 F3000   ; move Z up a bit and retract filament even more
G90                            ; absolute positioning
G1 X0 Y250                     ; move to cooling position
M84                            ; steppers off
G90                            ; absolute positioning
M117 TAZ Ready.
;CURA_PROFILE_STRING:""")
